...
public class MainActivity extends AppCompatActivity  implements StudentsFragment.StudentsListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportFragmentManager().beginTransaction().add(R.id.rootView, new StudentsFragment()).commit();
    }

    @Override
    public void sendSelectedStudent(Student student) {
        getSupportFragmentManager().beginTransaction()
                .replace(R.id.rootView,  StudentHistoryFragment.newInstance(student)).addToBackStack(null)
                .commit();
    }
}